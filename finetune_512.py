...
class T2IItemProcessor(ItemProcessor):
    ...
    def process_item(self, data_item, training_mode=False):
        if "prompt" in data_item:
            ...
            # root_path = "/mnt/weka/yt_workspace/Lumina-Image-2.0/images_pdd"
            # image = Image.open(os.path.join(root_path, url))
...
def set_trainable_params(model, trainable_param_names=None, verbose=True, logger=None):
    """
    Set requires_grad for parameters based on their names.
    
    Args:
        model: The PyTorch model
        trainable_param_names: List of strings. Parameters whose names contain
                             any of these strings will be set to trainable.
                             If None, all parameters will be trainable.
        verbose: If True, print parameter statistics
    
    Returns:
        trainable_params: List of trainable parameters
    """
    if trainable_param_names is None:
        # If no specific names provided, all parameters are trainable
    ...
    for name, param in model.named_parameters():
        ...
        # 检查参数名是否包含任何指定的关键词
        should_train = any(keyword in name for keyword in trainable_param_names)
    ...
    if verbose:
        ...
        logger.info(f"Frozen parameters: {total_count - trainable_count}")
        logger.info("\nTrainable parameter patterns:")
...
def encode_prompt(prompt_batch, text_encoder, tokenizer, proportion_empty_prompts, is_train=True, device=None):
    captions = []
...
def main(args):
    ...
    # ).cuda()
    model_id = "/hpc2hdd/home/sfei285/datasets/yetian/Lumina-Image-2-FT/model_weights/gemma-3-4b-it"
    ...
    vae = AutoencoderKL.from_pretrained("/hpc2hdd/home/sfei285/datasets/yetian/Lumina-Image-2-FT/model_weights/flux.1-dev-vae", torch_dtype=torch.bfloat16).to(
        device
    )

    trainable_params = set_trainable_params(model,trainable_param_names=None,verbose=True, logger=logger)
    ...
    for step in range(resume_step, args.max_steps):
        ...
        for train_res, data_pack in data_collection.items():
            ...
            with torch.no_grad():
                ...
                # import pdb; pdb.set_trace()
                cap_feats, cap_mask = encode_prompt(caps, text_encoder, tokenizer, args.caption_dropout_prob, device=device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--global_bsz_512", type=int, default=32)
    parser.add_argument("--micro_bsz_512", type=int, default=16)
